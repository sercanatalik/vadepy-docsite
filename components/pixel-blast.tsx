...
void main(){
  ...
  if(uEdgeFade>0.0){ vec2 norm=gl_FragCoord.xy/uResolution; float edge=min(min(norm.x,norm.y),min(1.0-norm.x,1.0-norm.y)); M*=smoothstep(0.0,uEdgeFade,edge); }
  vec3 color = uColor;
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (!threeRef.current) mustReinit = true;
      ...
      if (prevConfigRef.current.antialias !== cfg.antialias || prevConfigRef.current.liquid !== cfg.liquid || prevConfigRef.current.noiseAmount !== cfg.noiseAmount) mustReinit = true;
    }
    if (mustReinit) {
      if (threeRef.current) {
